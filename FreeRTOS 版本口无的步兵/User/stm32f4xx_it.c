...
void USART2_IRQHandler(void)
	...
	//	uint16_t DMA_Counter = DMA_GetCurrDataCounter(USART2_RX_DMA_STREAM);
		...
		{
			/*旧版格式*/
//			if(sscanf(CV_RXBUFF, "S%d,%d", &x,&y) == 2)
			/*新版格式*/
			if(sscanf(CV_RXBUFF, "S%d,%d", &x,&y) == 2)
			{
				/*测试*/
				...
				{
					Cloud.YAW_Speed_pid.Kp = -81.0f;
				}
				else
				{
					Cloud.YAW_Speed_pid.Kp = -82.0f;
				}
	
				
				if(abs(y)>0 && abs(y)<=30)
				{
					Cloud.Roll_Speed_pid.Kp = -21.0f;
				}
				else if(abs(y)>30 && abs(y)<=50)
				{
					Cloud.Roll_Speed_pid.Kp = -22.5f;
				}
				else if(abs(y)>50 && abs(y)<=70)
				{
					Cloud.Roll_Speed_pid.Kp = -24.0f;
				}
				else if(abs(y)>70 && abs(y)<=90)
				{
					Cloud.Roll_Speed_pid.Kp = -25.5f;
				}
				else if(abs(y)>90 && abs(y)<=110)
				{
					Cloud.Roll_Speed_pid.Kp = -27.0f;
				}
				else if(abs(y)>110 && abs(y)<=130)
				{
					Cloud.Roll_Speed_pid.Kp = -29.0f;
				}
				else
				{
...
//			if(sscanf(CV_RXBUFF, "S%d,", &x) == 1)
//			{
//				cv_x = x;
//				
//			}
//			if(sscanf(CV_RXBUFF, "N%dE", &y) == 1)
//			{
//				cv_y = y;
//				
//			}
