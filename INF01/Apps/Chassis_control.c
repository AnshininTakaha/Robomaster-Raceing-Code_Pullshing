...
void Chassis_Init(void)
{
	...
	/*PC控制的基础模式设置*/
	Chassis.pc_control_ChassisMode = Chassis_Normal;
	
	
	/*底盘基本PID初始化*/
	IncrementalPID_Init(&RM3508s[0].pid_wheel, 2.5f, 10.5f, 0.0f, 8000, 1000);
	IncrementalPID_Init(&RM3508s[1].pid_wheel, 2.5f, 10.5f, 0.0f, 8000, 1000);
	IncrementalPID_Init(&RM3508s[2].pid_wheel, 2.5f, 10.5f, 0.0f, 8000, 1000);
	IncrementalPID_Init(&RM3508s[3].pid_wheel, 2.5f, 10.5f, 0.0f, 8000, 1000);
...
void Chassis_Process(float Vx,float Vy,float VOmega,uint8_t mode)
{
	...
	/*底盘闭环（面对炮台归位）*/
	if(Chassis.pc_control_ChassisMode == Chassis_Fellow)
	{
		Chassis.FollowYawRaw = 0.0f;
		if( (abs(RM6623s[0].totalAngle - Cloud_Yaw_Center) > 682 || ( abs(Vy)+abs(Vx) )> 20.0f))
      {
            /*相对于云台需要的跟随的角度*/
